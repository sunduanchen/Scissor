...
This Scissor R package contains the proposed Scissor algorithm (function `Scissor`), which is a novel single-cell data analysis approach that utilizes bulk phenotypes to identify the most highly phenotype-associated cell subpopulations from single-cell RNA-seq (scRNA-seq) data. In this tutorial (R version: 3.6.1), we use several examples to help users executing Scissor in real applications. 
...
infos1$Scissor_neg
```
We can get access to the final model parameters using
```{r}
infos1$para
```
...
```
In the above implementation, we set a percentage cutoff (`cutoff = 0.03`), which stopped the α searching when the total percentage of the selected cells is less than 3%. The corresponding α equals 0.2, which selected 78 Scissor+ cells and 5 Scissor- cells. To avoid arbitrarily choosing alpha in real applications, we recommend searching α based on the percentage of the selected cells in total cells. The default value of `cutoff` is 0.2, i.e., the number of the Scissor selected cells should not exceed 20% of total cells in the single-cell data. Furthermore, users can set their customized α sequence and percentage cutoff to achieve different goals. For example, we can run Scissor as:
```{r warning=FALSE}
...
```{r pressure, echo=FALSE, fig.cap="", fig.align="center", out.width = '30%'}
...
Finally, we implemented Scissor to output cell level evaluations for each Scissor selected cell (function name: evaluate.cell). Now Scissor can report the mean correlations and the percentages of positive/negative correlations of a selected cell with all bulk samples. The percentage of positive correlation greater than 50% corresponds to in the updated criterion. 
```{r}
evaluate_result <- evaluate.cell('Scissor_LUAD_survival.RData', infos1, FDR = 0.05, bootstrap_n = 100)
```

```{r}
colnames(evaluate_result)
```

```{r}
evaluate_result[1:5,1:4]
```
the standard correlation p-value should also be inspected to assure the validity of the Scissor result
```{r}
evaluate_result[1:5,5:11]
```
we used the nonparametric bootstrap strategy to assess the sampling distribution of coefficient 𝜷 for all the Scissor selected cells. 

Using the application on the LUAD cancer cells with TP53 mutation as an example, we showed the evaluation results on ten example cells. By utilizing the bootstrap resampling, we can assess the cell level metrics such as the coefficient distribution. As you suggested, we added this utility into our Scissor package (function name: evaluate.cell). This function will output the five-number summary to show the range of the beta coefficient for each Scissor selected cell, which includes the minimum value (0%), the lower quartile (25%), the median value (50%), the upper quartile (75%), and the maximum value (100%). 
