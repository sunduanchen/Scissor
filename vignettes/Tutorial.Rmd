...
Scissor can associate cells with phenotypes, and this kind of association is a relative concept between phenotypes. Namely, Scissor assigns which phenotype a cell is more likely associated with than the other phenotype. Considering the possible negative correlations between the single-cell and bulk samples, we can further interpret a cell by assigning it into the following two categories (Figure 1): if the sum of a cell’s correlations with bulk samples is greater than 0, this cell is more similar to the associated phenotype. Otherwise, this cell should be interpreted as more dissimilar to the other phenotype. In most cases, the identified cells fall into the “more similar” category, and the negative correlations would not affect the interpretations of Scissor.

```{r pressure, echo=FALSE, fig.cap="Figure 1. General criterion to interpret phenotypes.", fig.align="center", out.width = '45%'}
...
result1 <- reliability.test(X, Y, network, alpha = 0.05, family = "cox", cell_num = numbers, n = 1, nfold = 10)
```
...
result2 <- reliability.test(X, Y, network, alpha = 0.2, family = "binomial", cell_num = numbers, n = 1, nfold = 10)
```
