...
This Scissor R package contains the proposed Scissor algorithm (function `Scissor`), which is a novel single-cell data analysis approach that utilizes bulk phenotypes to identify the most highly phenotype-associated cell subpopulations from single-cell sequencing data. In this tutorial (R version: 3.6.1), we use several examples to help users executing Scissor in real applications. 
