...
#' @export
Scissor.glmnet <- function(bulk_dataset, sc_dataset, phenotype, tag = NULL,
    ...
    for (i in 1:length(alpha)){
        set.seed(1)
        fit0 <- cv.glmnet(X, Y, family = 'cox', alpha = alpha[i], standardize = F, nlambda = 100, type.measure = 'deviance')  # C deviance
        fit1 <-    glmnet(X, Y, family = 'cox', alpha = alpha[i], standardize = F, lambda = fit0$lambda.min)
        if (family == "binomial"){
